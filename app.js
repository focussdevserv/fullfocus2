...
async function refreshHomePanels() {
  try {
    ...
    const taskList = document.querySelector(".priorities-card .priority-list"), agendaList = document.querySelector(".agenda-card .agenda-list");
...
async function renderAgendaView() {
  ...
  dashboardGrid.querySelectorAll(".calendar-day:not(.is-empty)").forEach((day) => day.addEventListener("click", () => { openCreateDialog("evento"); const input = dialogFields.querySelector('[name="startsAt"]'); const date = new Date(year, month, Number(day.dataset.day), 9, 0); if (input) input.value = new Date(date - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16); }));
