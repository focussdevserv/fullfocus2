...
const createModuleByKind = { contato: "contatos", cliente: "clientes", lead: "crm", proposta: "propostas", contrato: "contratos", receita: "receitas", projeto: "projetos", ticket: "tickets", recebivel: "contas-a-receber", assinatura: "assinaturas" };
async function openGlobalCreate(kind) {
  const moduleKey = createModuleByKind[kind];
  if (moduleKey && window.FocusModuleLoader?.load) await window.FocusModuleLoader.load(moduleKey);
  if (kind === "proposta" && typeof window.FocusOpenProposal === "function") return window.FocusOpenProposal();
  if (["contato", "cliente"].includes(kind) && typeof window.prepareCreate === "function") return window.prepareCreate(kind);
  if (!createConfig[kind]) throw new Error("Este formulário ainda não está disponível. Atualize a página e tente novamente.");
  if (["recebivel", "assinatura"].includes(kind)) {
    const clients = (await api("/api/clients")).clients || [];
    const clientField = createConfig[kind]?.fields?.find((field) => field.name === "client_id");
    if (clientField) clientField.options = [["", "Sem cliente"], ...clients.map((client) => [client.id, client.name])];
  }
  openCreateDialog(kind);
}
window.FocusOpenGlobalCreate = openGlobalCreate;
document.querySelectorAll(".create-menu a").forEach((link) => link.addEventListener("click", async (event) => {
  event.preventDefault(); createMenu.hidden = true; createMenuTrigger?.setAttribute("aria-expanded", "false");
  const text = link.textContent.toLocaleLowerCase("pt-BR");
  const kind = link.dataset.createKind || (text.includes("tarefa") ? "tarefa" : text.includes("lead") ? "lead" : text.includes("receita") ? "receita" : "projeto");
  link.setAttribute("aria-busy", "true");
  try { await openGlobalCreate(kind); } catch (error) { ui.toast(error.message || "Não foi possível abrir o cadastro.", "error"); }
  finally { link.removeAttribute("aria-busy"); }
}));
dialogForm.addEventListener("submit", async (event) => { event.preventDefault(); if (dialogForm.dataset.busy === "1") return; const config = createConfig[dialogForm.dataset.kind]; if (!config) return; const request = dialogRequest, routeAtStart = window.location.hash; const payload = Object.fromEntries(new FormData(dialogForm)); dialogForm.querySelectorAll('input[type="checkbox"]').forEach((input) => { payload[input.name] = input.checked; }); if (payload.amount) payload.amount = payload.amount.replace(",", "."); dialogForm.querySelectorAll('input[type="datetime-local"]').forEach((input) => { if (input.value) payload[input.name] = new Date(input.value).toISOString(); }); Object.keys(payload).forEach((key) => { if (payload[key] === "") delete payload[key]; }); const submit = dialogForm.querySelector("[type=submit]"); dialogForm.dataset.busy = "1"; submit.disabled = true; submit.setAttribute("aria-busy", "true"); dialogStatus.setAttribute("role", "status"); dialogStatus.setAttribute("aria-live", "polite"); dialogStatus.textContent = "Salvando…"; try { const method = dialogForm.dataset.method || "POST", endpoint = dialogForm.dataset.endpoint || config.endpoint; const response = await fetch(endpoint, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const data = response.status === 204 ? {} : await response.json(); if (!response.ok) throw new Error(data.error || "Não foi possível salvar."); if (request !== dialogRequest || createDialog.hidden || routeAtStart !== window.location.hash) return; closeCreateDialog(); /* Re-renderiza a tela atual (qualquer módulo) e atualiza o painel inicial. */ renderHashRoute(window.location.hash); if (method === "POST" && typeof config.onCreated === "function") await config.onCreated(data, payload); if ((!window.location.hash || window.location.hash === "#inicio") && !routeRenderers.inicio) await syncDashboard(); } catch (error) { if (request === dialogRequest && !createDialog.hidden) { dialogStatus.setAttribute("role", "alert"); dialogStatus.textContent = error.message || "Não foi possível salvar. Tente novamente."; } } finally { if (request === dialogRequest) { delete dialogForm.dataset.busy; submit.disabled = false; submit.removeAttribute("aria-busy"); } } });
...
/* Formulários longos começam enxutos; os campos complementares ficam sob demanda. */
const dialogAdvancedObserver = new MutationObserver(() => {
  if (dialogFields.querySelector("[data-dialog-advanced]")) return;
  const fields = [...dialogFields.children].filter((node) => node.matches?.(".dialog-field"));
  if (fields.length < 7) return;
  const details = document.createElement("details"); details.className = "dialog-advanced"; details.dataset.dialogAdvanced = "1";
  details.innerHTML = "<summary>Mais detalhes (opcional)</summary><div class=\"dialog-advanced-fields\"></div>";
  const target = details.querySelector(".dialog-advanced-fields");
  fields.slice(5).forEach((field) => target.append(field));
...
async function renderSearchResults(query) {
  ...
  const sources = [
    ...
    ["leads", "/api/leads", "leads", "name", "#leads"],
    ["oportunidades", "/api/opportunities", "opportunities", "name", "#oportunidades"],
    ["propostas", "/api/proposals", "proposals", "title", "#propostas"],
    ["contratos", "/api/contracts", "contracts", "name", "#contratos"],
    ["tickets", "/api/tickets", "tickets", "title", "#tickets"],
    ["infraestrutura", "/api/infrastructure_assets", "infrastructure_assets", "name", "#infraestrutura"]
  ];
