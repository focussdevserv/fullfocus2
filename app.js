...
async function renderTasksView(filter = "all", mode = taskViewMode) {
  ...
  const card = (task) => { const priority = task.priority || "medium"; return `<article class="kanban-task" draggable="true" data-kanban-id="${task.id}"><strong>${escapeHtml(task.title)}</strong><small>${task.due_at ? new Date(task.due_at).toLocaleDateString("pt-BR") : "Sem prazo"}</small><span class="task-status priority-${priority}">${priority === "high" ? "Alta" : priority === "low" ? "Baixa" : "Média"}</span><div><button class="task-edit" type="button" data-edit-task="${task.id}">✎</button><button class="task-delete" type="button" data-delete-task="${task.id}">×</button></div></article>`; };
  ...
  dashboardGrid.querySelectorAll("[data-edit-task]").forEach((button) => button.addEventListener("click", () => { const task = tasks.find((item) => String(item.id) === button.dataset.editTask); if (task) openEditDialog("tarefa", { title: task.title, priority: task.priority || "medium", dueAt: task.due_at }, `/api/tasks/${task.id}/details`); }));
...
async function renderAgendaView() {
  ...
  dashboardGrid.querySelectorAll("[data-edit-event]").forEach((button) => button.addEventListener("click", () => { const event = selectedEvents.find((item) => String(item.id) === button.dataset.editEvent); if (event) openEditDialog("evento", { title: event.title, startsAt: event.starts_at, description: event.description || "" }, `/api/events/${event.id}/details`); }));
