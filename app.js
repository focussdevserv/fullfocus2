...
document.querySelectorAll(".create-menu a").forEach((link) => link.addEventListener("click", (event) => { event.preventDefault(); createMenu.hidden = true; createMenuTrigger?.setAttribute("aria-expanded", "false"); const text = link.textContent.toLocaleLowerCase("pt-BR"); const kind = link.dataset.createKind || (text.includes("tarefa") ? "tarefa" : text.includes("lead") ? "lead" : text.includes("receita") ? "receita" : "projeto"); const prepare = window.prepareCreate; if (["contato", "cliente"].includes(kind) && typeof prepare === "function") prepare(kind); else openCreateDialog(kind); }));
