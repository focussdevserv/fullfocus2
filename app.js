...
async function refreshHomePanels() {
  try {
    ...
    if (taskList && tasks.length) taskList.innerHTML = tasks.slice(0, 5).map((task) => `<label class="priority-item"><input type="checkbox" data-home-task="${task.id}" ${task.status === "done" ? "checked" : ""} /><span class="checkmark"></span><span class="priority-text"><strong>${escapeHtml(task.title)}</strong><small>${task.due_at ? new Date(task.due_at).toLocaleString("pt-BR", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "Sem prazo"}</small></span><em class="priority-${task.priority || "medium"}">${task.priority === "high" ? "Alta" : task.priority === "low" ? "Baixa" : "Média"}</em></label>`).join("");
...
const createConfig = {
  tarefa: { title: "Nova tarefa", endpoint: "/api/tasks", fields: [{ name: "title", label: "Título", placeholder: "Ex.: Revisar briefing do cliente" }, { name: "priority", label: "Prioridade", type: "select", options: [["low", "Baixa"], ["medium", "Média"], ["high", "Alta"]] }, { name: "dueAt", label: "Prazo (opcional)", type: "datetime-local", placeholder: "", required: false }] },
...
dialogForm.addEventListener("submit", async (event) => { event.preventDefault(); const config = createConfig[dialogForm.dataset.kind]; const payload = Object.fromEntries(new FormData(dialogForm)); if (payload.amount) payload.amount = payload.amount.replace(",", "."); const submit = dialogForm.querySelector("[type=submit]"); submit.disabled = true; dialogStatus.textContent = "Salvando..."; try { const response = await fetch(config.endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Não foi possível salvar."); const wasAgenda = dialogForm.dataset.kind === "evento", wasTasks = dialogForm.dataset.kind === "tarefa"; closeCreateDialog(); if (wasAgenda) await renderAgendaView(); else if (wasTasks) await renderTasksView(); else await syncDashboard(); } catch (error) { dialogStatus.textContent = error.message; } finally { submit.disabled = false; } });
...
async function renderTasksView(filter = "all") {
  let tasks = [];
  ...
  dashboardGrid.querySelectorAll("[data-edit-task]").forEach((button) => button.addEventListener("click", async () => { const task = tasks.find((item) => String(item.id) === button.dataset.editTask); if (!task) return; const title = window.prompt("Nome da tarefa", task.title); if (title === null || !title.trim()) return; const priority = window.prompt("Prioridade: low, medium ou high", task.priority || "medium"); if (!priority || !["low", "medium", "high"].includes(priority)) return; const response = await fetch(`/api/tasks/${task.id}/details`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: title.trim(), priority, dueAt: task.due_at }) }); if (response.ok) renderTasksView(filter); }));
}
...
async function renderAgendaView() {
  ...
  const eventMarkup = selectedEvents.length ? selectedEvents.map((event, index) => { const date = new Date(event.starts_at); return `<article class="day-event" draggable="true" data-event-id="${event.id}"><time>${date.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</time><div><strong>${escapeHtml(event.title)}</strong><small>${escapeHtml(event.description || "Compromisso da agenda")}</small></div><i class="event-dot ${["blue", "pink", "orange"][index % 3]}"></i><button class="event-edit" type="button" data-edit-event="${event.id}" aria-label="Editar evento">✎</button><button class="event-delete" type="button" data-delete-event="${event.id}" aria-label="Excluir evento">×</button></article>`; }).join("") : '<p class="agenda-empty">Nenhum evento neste dia.</p>';
  dashboardGrid.innerHTML = `<section class="page-intro agenda-intro"><div><p class="card-kicker">Organização</p><h2>Agenda</h2><p>Planeje compromissos, reuniões e os próximos passos da equipe.</p></div><button class="button button-primary compact-action agenda-new" type="button">+ Novo evento</button></section><section class="agenda-layout"><section class="data-card calendar-card"><div class="calendar-toolbar"><button class="calendar-nav" type="button" data-agenda-shift="-1" aria-label="Mês anterior">‹</button><h2>${agendaMonth.toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</h2><button class="calendar-nav" type="button" data-agenda-shift="1" aria-label="Próximo mês">›</button></div><div class="calendar-weekdays"><span>Dom</span><span>Seg</span><span>Ter</span><span>Qua</span><span>Qui</span><span>Sex</span><span>Sáb</span></div><div class="calendar-grid">${cells}</div></section><aside class="data-card day-agenda"><div class="section-heading"><div><p class="card-kicker">${agendaSelectedDate.toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" })}</p><h2>Agenda do dia</h2></div><span class="task-count">${selectedEvents.length} evento${selectedEvents.length === 1 ? "" : "s"}</span></div><div class="day-event-list">${eventMarkup}</div><button class="agenda-add-link" type="button">+ Adicionar compromisso</button></aside></section><section class="data-card agenda-upcoming"><div class="section-heading"><div><p class="card-kicker">Visão geral</p><h2>Próximos compromissos</h2></div></div><div class="upcoming-list">${events.filter((event) => new Date(event.starts_at) >= now).slice(0, 5).map((event) => { const date = new Date(event.starts_at); return `<article><span class="upcoming-date">${date.toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}<strong>${date.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</strong></span><div><strong>${escapeHtml(event.title)}</strong><small>${escapeHtml(event.description || "Compromisso da agenda")}</small></div></article>`; }).join("") || '<p class="agenda-empty">Nenhum próximo compromisso.</p>'}</div></section>`;
  ...
  dashboardGrid.querySelectorAll("[data-edit-event]").forEach((button) => button.addEventListener("click", async () => { const event = selectedEvents.find((item) => String(item.id) === button.dataset.editEvent); if (!event) return; const title = window.prompt("Título do evento", event.title); if (title === null || !title.trim()) return; const response = await fetch(`/api/events/${event.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: title.trim(), startsAt: event.starts_at, description: event.description || "" }) }); if (response.ok) renderAgendaView(); }));
  ...
  dashboardGrid.querySelectorAll(".calendar-day:not(.is-empty)").forEach((day) => day.addEventListener("click", () => { agendaSelectedDate = new Date(year, month, Number(day.dataset.day)); renderAgendaView(); }));
  ...
  dashboardGrid.querySelectorAll(".calendar-day:not(.is-empty)").forEach((day) => {
