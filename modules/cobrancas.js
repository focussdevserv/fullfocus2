...
async function legacyChargeForm(item = null) {
...
function pixChargeDetails(item) {
  const pixDocument = item?.pix_payload ? `<section class="pix-charge-document"><strong>Pagamento exclusivamente via Pix</strong><img src="${chargeEsc(item.pix_qr_data_url)}" alt="QR Code Pix para pagamento"><label>Pix copia e cola<textarea readonly>${chargeEsc(item.pix_payload)}</textarea></label><small>Este documento nao possui codigo de barras. O pagamento deve ser feito pelo QR Code ou pelo codigo Pix copia e cola.</small></section>` : "";
  ui.drawer({ title: item?.message || "Cobranca", subtitle: chargeStatus[item?.status] || "Financeiro", html: ui.facts([["Mensagem", item?.message], ["Canal", item?.channel], ["Valor", chargeMoney(item?.amount)], ["Vencimento", chargeDate(item?.due_at)], ["Recebivel", item?.receivable_id], ["Provedor", item?.provider], ["Link de pagamento", item?.payment_url], ["Status", chargeStatus[item?.status] || item?.status]]) + pixDocument });
}

async function chargeFormPix(item = null) {
  const routeAtStart = location.hash;
  if (routeAtStart !== "#cobrancas") return;
  let receivables = [];
  if (!item) {
    try { receivables = (await api("/api/receivables")).receivables || []; } catch (error) { if (location.hash === routeAtStart) toast(error.message || "Nao foi possivel carregar as contas a receber.", "error"); return; }
  }
  if (location.hash !== routeAtStart || location.hash !== "#cobrancas") return;
  ui.form({ title: item ? "Editar cobranca" : "Nova cobranca", subtitle: "Financeiro", values: item || {}, fields: [{ name: "receivable_id", label: "Conta a receber", type: "select", required: false, options: [["", "Sem vinculo"], ...receivables.map((entry) => [entry.id, `${entry.description} - ${chargeMoney(entry.amount)}`])] }, { name: "amount", label: "Valor", type: "number", min: 0.01, step: 0.01 }, { name: "channel", label: "Canal", type: "select", options: [["pix", "Pix"], ["boleto", "Boleto Pix (sem codigo de barras)"], ["card", "Cartao"], ["link", "Link de pagamento"], ["transfer", "Transferencia"]] }, { name: "due_at", label: "Vencimento", type: "date", required: false }, { name: "message", label: "Mensagem", required: false }, { name: "status", label: "Status", type: "select", options: Object.entries(chargeStatus) }], onSubmit: async (values) => { let result; if (!item && values.receivable_id && ["pix", "boleto"].includes(values.channel)) result = await api(`/api/receivables/${values.receivable_id}/create-charge`, { method: "POST", body: { channel: values.channel, message: values.message } }); else result = await api(item ? `/api/charges/${item.id}` : "/api/charges", { method: item ? "PATCH" : "POST", body: values }); if (location.hash !== routeAtStart || location.hash !== "#cobrancas") return; toast(result?.notice || (item ? "Cobranca atualizada." : "Cobranca criada."), "success"); renderChargesScreen(); if (result?.charge?.pix_payload) setTimeout(() => pixChargeDetails(result.charge), 0); } });
}
...
async function renderChargesScreen() {
  ...
  try {
    ...
    dashboardGrid.querySelector(".finance-list .ui-empty .compact-action")?.addEventListener("click", () => chargeFormPix());
    dashboardGrid.querySelectorAll("[data-charge-edit]").forEach((button) => button.addEventListener("click", () => { const item = items.find((entry) => String(entry.id) === String(button.dataset.chargeEdit)); if (item) chargeFormPix(item).catch((error) => toast(error.message, "error")); }));
    dashboardGrid.querySelectorAll(".finance-list tbody tr").forEach((row, index) => { const button = document.createElement("button"); button.className = "compact-action"; button.type = "button"; button.textContent = "Detalhes"; button.addEventListener("click", () => items[index]?.pix_payload ? pixChargeDetails(items[index]) : chargeDetails(items[index])); row.querySelector("td:last-child")?.prepend(button); }); dashboardGrid.querySelector(".finance-list")?.setAttribute("data-details-bound", "1");
...
dashboardGrid.addEventListener("click", (event) => {
  ...
  chargeFormPix().catch((error) => toast(error.message, "error")).finally(() => {
