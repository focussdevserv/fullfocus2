...
const hubMojibake = new Map([
  ["Ã§", "ç"], ["Ã£", "ã"], ["Ãµ", "õ"], ["Ã¡", "á"], ["Ã©", "é"], ["Ãª", "ê"], ["Ã­", "í"], ["Ã³", "ó"], ["Ã´", "ô"], ["Ãº", "ú"], ["Ã§Ãµ", "ções"],
  ["Ã‰", "É"], ["ÃŠ", "Ê"], ["Ã“", "Ó"], ["Ãš", "Ú"], ["â€¦", "…"], ["â†’", "→"], ["âŒ•", "⌕"], ["â–¦", "▪"], ["â–£", "▣"], ["â—‰", "◉"], ["â–¡", "□"], ["â™§", "♧"], ["â–¤", "▫"], ["â™™", "♙"], ["â¬¢", "⬢"], ["â—ˆ", "◈"],
]);
const repairHubEncoding = (value) => [...hubMojibake].reduce((text, [broken, fixed]) => String(text).split(broken).join(fixed), String(value));
const repairHubVisibleText = (root) => {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
  const textNodes = [];
  while (walker.nextNode()) textNodes.push(walker.currentNode);
  textNodes.forEach((node) => { node.nodeValue = repairHubEncoding(node.nodeValue); });
  root.querySelectorAll("[aria-label], [placeholder], [title]").forEach((element) => ["aria-label", "placeholder", "title"].forEach((attribute) => {
    if (element.hasAttribute(attribute)) element.setAttribute(attribute, repairHubEncoding(element.getAttribute(attribute)));
  }));
};
...
async function renderSettingsHub() {
  ...
  try {
    ...
    repairHubVisibleText(dashboardGrid);
    ...
    dashboardGrid.querySelectorAll("[data-hub-section]").forEach((button) => button.addEventListener("click", () => {
      const target = hubRouteMap[button.dataset.hubSection];
      if (activateHubSection(target)) return;
      if (target && target !== "configuracoes") location.hash = `#${target}`;
    }));
