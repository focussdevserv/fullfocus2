...
function setFinanceClientOptions(kind, clients) { const field = createConfig[kind]?.fields?.find((item) => item.name === "client_id"); if (!field) throw new Error("Este formulário ainda não está disponível. Atualize a tela e tente novamente."); field.type = "select"; field.options = [["", "Sem cliente"], ...clients.map((client) => [client.id, client.name])]; }
function bindCreate() { dashboardGrid.querySelectorAll("[data-create]").forEach((button) => button.addEventListener("click", async () => { if (["recebivel", "assinatura"].includes(button.dataset.create)) { const clients = (await api("/api/clients")).clients || []; setFinanceClientOptions(button.dataset.create, clients); } openCreateDialog(button.dataset.create); })); }
function bindRetry() { dashboardGrid.querySelector(".state-retry")?.addEventListener("click", () => renderHashRoute()); }
function rowActions(items, table) { return items.map((item) => { const status = item.status || (item.paid_at ? "paid" : "pending"), canPay = ["pending", "partially_paid"].includes(status); const paymentInput = ""; return `<tr data-finance-row="${financeEsc(item.id)}"><td>${financeEsc(item.description)}</td><td>${financeCurrency(item.amount)}</td><td>${financeDate(item.due_at)}</td><td><span class="finance-status ${financeTone(status)}">${financeStatus[status] || financeEsc(status)}</span></td><td class="finance-actions">${canPay ? paymentInput + `<button type="button" class="compact-action" data-pay="${item.id}" data-table="${table}">Marcar pago</button>` : ""}<button type="button" class="compact-action" data-remove="${item.id}" data-table="${table}">Excluir</button></td></tr>`; }).join(""); }
...
async function renderLedgerConnected(table, title, description, kind, singular) {
  ...
  try {
    ...
    dashboardGrid.querySelector(".finance-list")?.insertAdjacentHTML("beforebegin", financeKpis(items)); bindCreate(); bindActions(); dashboardGrid.querySelectorAll("tbody tr[data-finance-row]").forEach((row) => { const item = items.find((entry) => String(entry.id) === String(row.dataset.financeRow)); if (!item) return; const detail = document.createElement("button"); detail.type = "button"; detail.className = "compact-action"; detail.textContent = "Detalhes"; detail.addEventListener("click", () => financeDetails(item, table)); row.querySelector(".finance-actions")?.prepend(detail); });
...
document.addEventListener("click", async (event) => {
  ...
  try {
    if (["recebivel", "assinatura"].includes(kind)) {
      ...
      setFinanceClientOptions(kind, clients);
    }
    if (location.hash !== routeAtStart || !button.isConnected) return;
