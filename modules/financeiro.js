...
async function renderReports(months = 6) { dashboardGrid.innerHTML = intro("Relatórios", "Compare receitas, despesas e resultado por período.") + state("loading", "Carregando relatório..."); try { const rows = (await api(`/api/finance/summary?months=${months}`)).summary || [], totalRevenue = rows.reduce((sum, row) => sum + Number(row.revenue || 0), 0), totalExpense = rows.reduce((sum, row) => sum + Number(row.expense || 0), 0), result = totalRevenue - totalExpense; dashboardGrid.innerHTML = intro("Relatórios", "Compare receitas, despesas e resultado por período.") + ui.stats([{ label: "Receitas no período", value: financeCurrency(totalRevenue), tone: "green" }, { label: "Despesas no período", value: financeCurrency(totalExpense), tone: "orange" }, { label: "Resultado acumulado", value: financeCurrency(result), tone: result >= 0 ? "green" : "red" }, { label: "Meses analisados", value: ui.number(rows.length) }]) + `<section class="data-card finance-list"><div class="finance-toolbar"><label>Período<select data-report-months aria-label="Período do relatório"><option value="3" ${months === 3 ? "selected" : ""}>3 meses</option><option value="6" ${months === 6 ? "selected" : ""}>6 meses</option><option value="12" ${months === 12 ? "selected" : ""}>12 meses</option><option value="24" ${months === 24 ? "selected" : ""}>24 meses</option></select></label><button class="button button-secondary compact-action" data-export type="button">Exportar CSV</button></div><div class="table-wrap"><table class="finance-table"><thead><tr><th>Mês</th><th>Receitas</th><th>Despesas</th><th>Resultado</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${financeEsc(row.month)}</td><td>${financeCurrency(row.revenue)}</td><td>${financeCurrency(row.expense)}</td><td>${financeCurrency(row.result)}</td></tr>`).join("") || `<tr><td colspan="4">Nenhuma movimentação no período.</td></tr>`}</tbody></table></div></section>`; dashboardGrid.querySelector("[data-report-months]")?.addEventListener("change", (event) => renderReports(Number(event.target.value))); dashboardGrid.querySelector("[data-export]")?.addEventListener("click", () => ui.downloadCsv("relatorio-financeiro.csv", ["Mês", "Receitas", "Despesas", "Resultado"], rows.map((row) => [row.month, row.revenue, row.expense, row.result]))); } catch (error) { dashboardGrid.innerHTML = intro("Relatórios", "Compare receitas, despesas e resultado por período.") + state("error", error.message); bindRetry(); } }
...
createConfig.cobranca = { title: "Nova cobrança", endpoint: "/api/charges", fields: [{ name: "receivable_id", label: "Recebível", type: "number", min: 1, step: 1 }, { name: "amount", label: "Valor", type: "number", min: 0.01, step: 0.01 }] };
...
const financeCreateKinds = new Set(["receita", "despesa", "recebivel", "cobranca", "assinatura"]);
