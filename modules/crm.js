  ...
  await wrap("Propostas", "CRM", (async () => {
    ...
    dashboardGrid.innerHTML = header({ kicker: "CRM", title: "Propostas", description: `${sent.length} aguardando resposta · ${money(sent.reduce((n, p) => n + Number(p.amount || 0), 0))}`, actions: button({ label: "+ Nova proposta", attr: "data-new" }) })
      ...
      + `<section class="data-card crm-card">${toolbar({ search: { value: proposalState.query, placeholder: "Buscar proposta…" }, filters: [
        ...
        { key: "title", label: "Proposta", render: (p) => `<strong>${esc(p.title)}</strong><small>${esc(p.client_name || p.project_name || p.opportunity_name || p.lead_name || "Sem vínculo")}${p.items_count ? ` · ${esc(p.items_count)} ${p.items_count === 1 ? "item" : "itens"}` : ""}</small>` },
  ...
  try {
    ...
    const render = () => {
      ...
      d.body.innerHTML = `
        ...
        ${facts([["Cliente", p.client_name], ["Projeto", p.project_name], ["Oportunidade", p.opportunity_name], ["Lead", p.lead_name], ["Validade", p.valid_until ? date(p.valid_until) : null], ["Enviada em", p.sent_at ? dateTime(p.sent_at) : null], ["Decidida em", p.decided_at ? dateTime(p.decided_at) : null]])}
        ...
        <div class="crm-drawer-actions">${p.status === "draft" ? button({ label: "Marcar como enviada", attr: 'data-status="sent"' }) : ""}${p.status === "sent" ? button({ label: "Aceita", attr: 'data-status="accepted"' }) + button({ label: "Recusada", kind: "secondary", attr: 'data-status="rejected"' }) : ""}${p.status === "accepted" ? button({ label: "Gerar contrato", attr: 'data-contract="true"' }) : ""}${p.client_id ? button({ label: "Abrir ficha do cliente", kind: "secondary", attr: 'data-client="true"' }) : ""}${button({ label: "Imprimir / PDF", kind: "secondary", attr: "data-print" })}${button({ label: "Editar", kind: "secondary", attr: "data-edit" })}</div>`;
