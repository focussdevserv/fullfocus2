...
// Corrige apenas texto visível que veio de bundles antigos com UTF-8 decodificado duas vezes.
const contaEncodingPairs = [["\u00c3\u00a1", "\u00e1"], ["\u00c3\u00a3", "\u00e3"], ["\u00c3\u00a7", "\u00e7"], ["\u00c3\u00a9", "\u00e9"], ["\u00c3\u00aa", "\u00ea"], ["\u00c3\u00ad", "\u00ed"], ["\u00c3\u00b3", "\u00f3"], ["\u00c3\u00b4", "\u00f4"], ["\u00c3\u00ba", "\u00fa"], ["\u00c3\u00b5", "\u00f5"], ["\u00c2\u00b7", "\u00b7"], ["\u00e2\u20ac\u201d", "\u2014"], ["\u00e2\u20ac\u00a6", "\u2026"], ["\u00e2\u2020\u2019", "\u2192"]];
const contaRepairEncoding = (root) => {
  if (!root) return;
  const repair = (value) => { let current = String(value ?? ""); for (let pass = 0; pass < 3; pass += 1) { const next = contaEncodingPairs.reduce((text, [from, to]) => text.replaceAll(from, to), current); if (next === current) break; current = next; } return current; };
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
  const textNodes = []; while (walker.nextNode()) textNodes.push(walker.currentNode);
  textNodes.forEach((node) => { const next = repair(node.nodeValue); if (next !== node.nodeValue) node.nodeValue = next; });
  root.querySelectorAll?.("[aria-label], [placeholder], [title]").forEach((element) => ["aria-label", "placeholder", "title"].forEach((attribute) => { if (element.hasAttribute(attribute)) { const next = repair(element.getAttribute(attribute)); if (next !== element.getAttribute(attribute)) element.setAttribute(attribute, next); } }));
};
if (typeof MutationObserver !== "undefined" && typeof dashboardGrid !== "undefined") { const contaEncodingObserver = new MutationObserver(() => contaRepairEncoding(dashboardGrid)); contaEncodingObserver.observe(dashboardGrid, { childList: true, subtree: true, characterData: true }); contaRepairEncoding(dashboardGrid); }
...
createConfig.cliente = { title: "Novo cliente", endpoint: "/api/clients/quick", fields: [field("name", "Nome", "text", { required: true }), field("company_id", "Empresa", "select", { options: [] }), field("contact_id", "Contato", "select", { options: [] }), field("company_name", "Empresa (opcional)"), field("company_document", "CNPJ da empresa (opcional)"), field("email", "E-mail", "email"), field("phone", "Telefone"), field("status", "Status", "select", { options: [["lead", "Lead"], ["prospecting", "Prospectando"], ["active", "Cliente ativo"], ["inactive", "Cliente inativo"], ["blocked", "Bloqueado"], ["churned", "Cancelado"]] })] };
...
async function hydrateClientOperationSections(overview, record, data, helpers) {
  ...
  if (!panel) return;
  const grids = [...panel.querySelectorAll(".client-sheet-grid")];
  ...
  const sections = [
    ["tasks", "Tarefas", "Nenhuma tarefa registrada.", (item) => `<div class="client-related-row"><strong>${contaEsc(item.title)}</strong><span>${contaEsc(helpers.label(item.status))}</span><small>${item.due_at ? `Prazo ${contaFieldValue("due_at", item.due_at)}` : "Sem prazo"}</small></div>`],
    ["files", "Arquivos", "Nenhum arquivo vinculado.", (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(item.kind || "Arquivo")}</span><small>${item.url ? `<a href="${contaEsc(item.url)}" target="_blank" rel="noopener noreferrer">Abrir arquivo</a>` : "Sem link"}</small></div>`],
    ["briefings", "Briefings", "Nenhum briefing registrado.", (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(helpers.label(item.status))}</span><small>Briefing do projeto</small></div>`],
    ["change_requests", "Alterações", "Nenhuma alteração registrada.", (item) => `<div class="client-related-row"><strong>${contaEsc(item.title)}</strong><span>${contaEsc(helpers.label(item.status))}</span><small>${item.additional_cost ? `Adicional ${helpers.money(item.additional_cost)}` : "Sem adicional informado"}</small></div>`],
    ["infrastructure", "Infraestrutura", "Nenhum recurso de infraestrutura.", (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(item.provider || item.kind || "Recurso")}</span><small>${item.expires_on ? `Vencimento ${contaFieldValue("expires_on", item.expires_on)}` : "Sem vencimento"}</small></div>`],
  ];
