...
openClientDetails = async function openCompleteClientSheet(record) {
  ...
  try {
    const data = await api(`/api/clients/${record.id}/overview`);
    if (!panel.isConnected) return;
    ...
    overview.innerHTML = `<nav class="client-sheet-tabs" aria-label="Seções da ficha"><button type="button" class="is-active" aria-selected="true" data-client-tab="sheet-overview">Resumo</button><button type="button" aria-selected="false" data-client-tab="sheet-profile">Dados e contatos</button><button type="button" aria-selected="false" data-client-tab="sheet-commercial">Comercial</button><button type="button" aria-selected="false" data-client-tab="sheet-projects">Projetos e operação</button><button type="button" aria-selected="false" data-client-tab="sheet-finance">Financeiro</button><button type="button" aria-selected="false" data-client-tab="sheet-history">Histórico</button></nav><section data-client-panel="sheet-overview"><div class="conta-overview-stats"><div><small>Conversas</small><strong>${data.conversations.length}</strong></div><div><small>Projetos ativos</small><strong>${data.summary.active_projects}</strong></div><div><small>Tarefas abertas</small><strong>${data.tasks.filter((item) => !["done", "completed"].includes(item.status)).length}</strong></div><div><small>Em aberto</small><strong>${money(openAmount)}</strong></div><div><small>Tickets abertos</small><strong>${data.summary.open_tickets}</strong></div><div><small>Alterações pendentes</small><strong>${data.summary.pending_changes}</strong></div></div><div class="client-sheet-grid"><article><h3>Próximas ações</h3>${rows([...data.tasks.filter((item) => !["done", "completed"].includes(item.status)).slice(0, 5).map((item) => ({ text: item.title, meta: label(item.status), href: "#tarefas" })), ...data.receivables.filter((item) => !item.paid_at).slice(0, 3).map((item) => ({ text: item.description, meta: money(item.amount), href: "#cobrancas" })), ...data.tickets.filter((item) => !["done", "closed", "cancelled"].includes(item.status)).slice(0, 2).map((item) => ({ text: item.title, meta: `Ticket · ${label(item.priority)}`, href: "#tickets" }))], (item) => `<a class="client-next-action" href="${item.href}">${contaEsc(item.text)} <span>${contaEsc(item.meta)}</span></a>`, "Nenhuma pendência para este cliente.")}</article><article><h3>Resumo cadastral</h3>${facts}</article></div></section><section data-client-panel="sheet-profile" hidden><div class="client-sheet-grid"><article><h3>Dados completos</h3>${facts}</article><article><h3>Contatos relacionados</h3>${rows(data.contacts, (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(item.role || "Contato")}</span><small>${contaEsc(item.email || item.phone || "Sem canal informado")}</small></div>`, "Nenhum contato vinculado.")}</article></div></section><section data-client-panel="sheet-commercial" hidden><div class="client-sheet-grid"><article><h3>Propostas</h3>${rows(data.proposals, (item) => `<div class="client-related-row"><strong>${contaEsc(item.title)}</strong><span>${contaEsc(label(item.status))}</span><small>${money(item.amount)} · validade ${contaFieldValue("valid_until", item.valid_until)}</small></div>`, "Nenhuma proposta registrada.")}</article><article><h3>Conversas</h3>${rows(data.conversations, (item) => `<div class="client-related-row"><strong>${contaEsc(item.subject)}</strong><span>${contaEsc(item.channel || "interno")} · ${contaEsc(label(item.status))}</span><small>${contaEsc(item.last_message || "Sem mensagens")}</small></div>`, "Nenhuma conversa vinculada.")}</article></div></section><section data-client-panel="sheet-projects" hidden><div class="client-sheet-grid"><article><h3>Projetos</h3>${rows(data.projects, (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(label(item.status))} · ${Number(item.progress || 0)}%</span><small>${item.due_on ? `Prazo ${contaFieldValue("due_on", item.due_on)}` : "Sem prazo definido"}${item.production_url ? ` · <a href="${contaEsc(item.production_url)}" target="_blank" rel="noopener noreferrer">Abrir</a>` : ""}</small></div>`, "Nenhum projeto vinculado.")}</article><article><h3>Tarefas, contratos e suporte</h3>${rows(data.tasks, (item) => `<div class="client-related-row"><strong>${contaEsc(item.title)}</strong><span>${contaEsc(label(item.status))}</span><small>${item.due_at ? `Prazo ${contaFieldValue("due_at", item.due_at)}` : "Sem prazo"}</small></div>`, "Nenhuma tarefa registrada.")}${rows(data.contracts, (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(label(item.status))}</span><small>${money(item.value)}${item.ends_on ? ` · até ${contaFieldValue("ends_on", item.ends_on)}` : ""}</small></div>`, "Nenhum contrato registrado.")}${rows(data.tickets, (item) => `<div class="client-related-row"><strong>${contaEsc(item.title)}</strong><span>${contaEsc(label(item.status))} · ${contaEsc(label(item.priority))}</span><small>${item.due_at ? `Prazo ${contaFieldValue("due_at", item.due_at)}` : "Sem prazo"}</small></div>`, "Nenhum ticket registrado.")}</article></div><div class="client-sheet-grid"><article><h3>Arquivos e briefings</h3>${rows(data.files, (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(item.kind || "Arquivo")}</span><small>${item.url ? `<a href="${contaEsc(item.url)}" target="_blank" rel="noopener noreferrer">Abrir arquivo</a>` : "Sem link"}</small></div>`, "Nenhum arquivo vinculado.")}${rows(data.briefings, (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(label(item.status))}</span><small>Briefing do projeto</small></div>`, "Nenhum briefing registrado.")}</article><article><h3>Alterações e infraestrutura</h3>${rows(data.change_requests, (item) => `<div class="client-related-row"><strong>${contaEsc(item.title)}</strong><span>${contaEsc(label(item.status))}</span><small>${item.additional_cost ? `Adicional ${money(item.additional_cost)}` : "Sem adicional informado"}</small></div>`, "Nenhuma alteração registrada.")}${rows(data.infrastructure, (item) => `<div class="client-related-row"><strong>${contaEsc(item.name)}</strong><span>${contaEsc(item.provider || item.kind || "Recurso")}</span><small>${item.expires_on ? `Vencimento ${contaFieldValue("expires_on", item.expires_on)}` : "Sem vencimento"}</small></div>`, "Nenhum recurso de infraestrutura.")}</article></div></section><section data-client-panel="sheet-finance" hidden><div class="client-sheet-grid"><article><h3>Contas a receber</h3>${rows(data.receivables, (item) => `<div class="client-related-row"><strong>${contaEsc(item.description)}</strong><span>${contaEsc(label(item.status))}</span><small>${money(item.amount)} · vencimento ${contaFieldValue("due_at", item.due_at)}${item.paid_at ? ` · pago em ${contaFieldValue("paid_at", item.paid_at)}` : ""}</small></div>`, "Nenhum lançamento financeiro.")}</article><article><h3>Resumo financeiro</h3>${ui.facts([["Em aberto", money(openAmount)], ["Projetos concluídos", data.summary.completed_projects], ["Mensalidade", money(data.summary.recurring_monthly)], ["Tickets abertos", data.summary.open_tickets]])}<a class="client-next-action" href="#cobrancas">Abrir cobranças <span>→</span></a></article></div></section><section data-client-panel="sheet-history" hidden><article><h3>Histórico de atividades</h3>${rows(data.activities, (item) => `<div class="client-related-row"><strong>${contaEsc(item.action)}</strong><span>${contaEsc(item.entity_type || "registro")}</span><small>${contaFieldValue("created_at", item.created_at)}</small></div>`, "Nenhuma atividade registrada.")}</article></section>`;
