...
async function hydrateClientOperationSections(overview, record, data, helpers) {
  ...
  await Promise.all(sections.map(async ([key, _title, emptyMessage, render]) => {
    ...
    try { const result = await api(`/api/clients/${record.id}/overview/section/${key}`); target.innerHTML = `<h3>${target.querySelector("h3")?.textContent || key}</h3>${helpers.rows(result[key] || [], render, emptyMessage)}`; }
...
openClientDetails = async function openCompleteClientSheet(record) {
  ...
  try {
    ...
    hydrateClientOperationSections(overview, record, data, { money, label, rows }).catch(() => {});
