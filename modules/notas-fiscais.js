...
const invoiceDetailsObserver = new MutationObserver(async () => { if (location.hash.replace(/^#/, "") !== "notas-fiscais") return; const list = dashboardGrid.querySelector(".finance-list"); if (!list || list.dataset.detailsBound === "1") return; list.dataset.detailsBound = "1"; try { const items = (await api("/api/invoices?limit=100")).invoices || []; if (location.hash !== "#notas-fiscais" || !list.isConnected || dashboardGrid.querySelector(".finance-list") !== list) return; list.querySelectorAll("tbody tr").forEach((row, index) => { const button = document.createElement("button"); button.type = "button"; button.className = "compact-action"; button.textContent = "Detalhes"; button.addEventListener("click", () => { const item = items[index]; if (!item) return; const html = Object.entries(item).filter(([key]) => !["id", "organization_id"].includes(key)).map(([key, value]) => `<dt>${invoiceEsc(key.replaceAll("_", " "))}</dt><dd>${invoiceEsc(value === null || value === undefined || value === "" ? "—" : typeof value === "object" ? JSON.stringify(value) : value)}</dd>`).join(""); ui.drawer({ title: item.number || "Nota fiscal", subtitle: invoiceStatus[item.status] || item.status || "Sem status", html: `<dl>${html}</dl>` }); }); row.querySelector("td:last-child")?.prepend(button); }); } catch { if (list.isConnected) list.dataset.detailsBound = ""; } });
