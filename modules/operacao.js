  ...
  try {
    ...
    const table = ui.table({
      columns: [
        ...
        { key: "progress", label: "Progresso", width: "160px", render: (x) => `<div class="operations-progress"><span style="width:${Math.max(0, Math.min(100, Number(x.progress) || 0))}%"></span></div><small>${Number(x.progress) || 0}%</small>` },
    ...
    dashboardGrid.innerHTML = intro(title, description, "projeto") + stats + `<section class="data-card operations-table">${toolbar}${universe.length ? table : state("empty", "Nenhum projeto", "Crie um projeto para acompanhar entregas.")}</section>`;
    const restoreFocus = ui.keepSearchFocus(dashboardGrid);
    ...
    dashboardGrid.querySelectorAll("[data-progress]").forEach((button) => button.addEventListener("click", () => { const item = rows.find((row) => String(row.id) === button.dataset.progress); if (!item) return; ui.form({ title: `Progresso · ${item.name}`, subtitle: "Projeto", fields: [{ name: "progress", label: "Progresso (%)", type: "number", min: 0, max: 100, step: 1, value: Number(item.progress) || 0 }, { name: "status", label: "Status", type: "select", value: item.status || "active", options: projectStatuses.map((status) => [status, labels[status] || status]) }], submitLabel: "Salvar", onSubmit: async (values) => { await api(`/api/projects/${item.id}`, { method: "PATCH", body: { progress: Math.max(0, Math.min(100, Number(values.progress) || 0)), status: values.status } }); ui.toast("Progresso atualizado.", "success"); renderProjectsConnected(); } }); }));
  } catch (error) { dashboardGrid.innerHTML = intro(title, description, "projeto") + state("error", error.message); }
}
