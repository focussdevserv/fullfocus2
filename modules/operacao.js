...
async function setup(kind) { const c = config[kind]; if (c.fields.some((f) => f.name.endsWith("_id"))) { const [clientData, contractData, proposalData] = await Promise.all([api("/api/clients"), kind === "projeto" ? api("/api/contracts") : Promise.resolve({ contracts: [] }), kind === "contrato" ? api("/api/proposals") : Promise.resolve({ proposals: [] })]); clients = clientData.clients || []; c.fields.forEach((f) => { if (f.name === "client_id") f.options = [["", "Sem cliente"], ...clients.map((x) => [x.id, x.name])]; if (f.name === "contract_id" && f.type === "select") f.options = [["", "Sem contrato"], ...(contractData.contracts || []).map((x) => [x.id, x.name])]; if (f.name === "proposal_id" && f.type === "select") { const proposals = (proposalData.proposals || []).filter((x) => x.status === "accepted"); f.options = [["", proposals.length ? "Selecione uma proposta aprovada" : "Nenhuma proposta aprovada"], ...proposals.map((x) => [x.id, `${x.title || "Proposta"} · ${money(x.amount)}`])]; } }); } openCreateDialog(kind); }
...
function bindCommon(kind, rows, endpoint) { dashboardGrid.querySelector(".operations-new")?.addEventListener("click", () => setup(kind)); dashboardGrid.querySelectorAll("[data-delete]").forEach((b) => b.addEventListener("click", () => ui.confirmInline(b, { text: "Excluir?", onConfirm: async () => { await api(`${endpoint}/${b.dataset.delete}`, { method: "DELETE" }); ui.toast("Registro excluído.", "success"); renderHashRoute(); } }))); dashboardGrid.querySelectorAll("[data-edit]").forEach((b) => b.addEventListener("click", () => { const item = rows.find((x) => String(x.id) === b.dataset.edit); if (!item) return; setup(kind).then(() => { const form = document.querySelector("#create-form"); if (!form) return; Object.entries(item).forEach(([k, v]) => { const field = form.elements[k]; if (field && v != null) field.value = String(v); }); form.dataset.method = "PATCH"; form.dataset.endpoint = `${endpoint}/${item.id}`; }); })); }
const contractsState = { search: "", status: "" };
...
async function renderContracts() {
  ...
  } catch (e) { dashboardGrid.innerHTML = intro(title, description, "contrato") + state("error", e.message); }
}
function openContractDetails(contract) {
  ...
  const facts = ui.facts([
    ...
    ["Parcelas", contract.installments], ["Entrada", contract.down_payment ? money(contract.down_payment) : ""], ["Assinado em", contract.signed_at ? new Date(contract.signed_at).toLocaleString("pt-BR") : ""],
    ["Criado em", contract.created_at ? new Date(contract.created_at).toLocaleString("pt-BR") : ""],
  ]);
  ...
  ui.drawer({
    ...
    onOpen: (body) => {
      ...
      const run = async (button, work) => { button.disabled = true; try { await work(); } catch (error) { say(error.message, true); } finally { button.disabled = false; } };
      body.querySelector("[data-contract-link]").addEventListener("click", (event) => run(event.currentTarget, async () => { const data = await api(`/api/contracts/${contract.id}/public-link`, { method: "POST", body: {} }); const url = `${location.origin}${data.path}`; try { await navigator.clipboard?.writeText(url); } catch { /* sem permissão de área de transferência */ } say(`Link copiado: ${url}`); ui.toast("Link de assinatura gerado.", "success"); }));
...
async function openProjectOverview(id) {
  const panel = document.createElement("aside"); panel.className = "conta-drawer conta-drawer-wide"; panel.innerHTML = '<button class="conta-close" type="button">×</button><p class="card-kicker">Projeto</p><h2>Carregando resumo...</h2><div class="conta-related">Carregando tarefas, arquivos e financeiro...</div>'; document.body.append(panel); panel.querySelector(".conta-close").onclick = () => panel.remove();
  try { const data = await api(`/api/projects/${id}/overview`), project = data.project || {}; panel.querySelector("h2").textContent = project.name || "Projeto"; panel.querySelector(".conta-related").innerHTML = `<section><h3>Status e progresso</h3><p>${esc(labels[project.status] || project.status || "—")} · ${Number(project.progress || 0)}%</p><div class="operations-progress"><span style="width:${Math.max(0, Math.min(100, Number(project.progress) || 0))}%"></span></div></section><section><h3>Resumo</h3><p>Cliente: ${esc(project.client_name || "Sem cliente")}</p><p>Prazo: ${date(project.due_on)}</p><p>A receber: ${money(data.balance_receivable)}</p><p>Tarefas atrasadas: ${data.overdue_tasks || 0}</p></section><section><h3>Tarefas</h3>${data.tasks?.length ? data.tasks.map((item) => `<p>${esc(item.title)} · ${esc(labels[item.status] || item.status || "Pendente")}</p>`).join("") : "<p>Nenhuma tarefa vinculada.</p>"}</section><section><h3>Arquivos</h3>${data.files?.length ? data.files.map((item) => `<p>${esc(item.name)}</p>`).join("") : "<p>Nenhum arquivo vinculado.</p>"}</section>`; } catch (error) { panel.querySelector(".conta-related").textContent = error.message; }
}
...
async function renderProjectsConnected() {
  const title = "Projetos"; dashboardGrid.innerHTML = intro(title, "Acompanhe progresso, status, clientes e contratos.", "projeto") + state("loading", "Carregando projetos...");
  try {
    ...
    const rows = (await api(`/api/projects?${params}`)).projects || [];
    dashboardGrid.innerHTML = intro(title, "Acompanhe progresso, status, clientes e contratos.", "projeto") + `<section class="data-card operations-table"><div class="operations-toolbar"><input class="operations-search-input" data-project-search value="${esc(projectState.query)}" type="search" placeholder="Buscar projeto, descrição ou cliente" aria-label="Buscar projeto, descrição ou cliente"><select class="operations-filter" data-project-status aria-label="Filtrar status"><option value="">Todos os status</option>${projectStatuses.map((status) => `<option value="${status}" ${projectState.status === status ? "selected" : ""}>${esc(labels[status] || status)}</option>`).join("")}</select></div><div class="table-wrap"><table><thead><tr><th>Projeto</th><th>Cliente / contrato</th><th>Progresso</th><th>Status</th><th>Ações</th></tr></thead><tbody>${rows.map((x) => `<tr><td>${esc(x.name)}</td><td>${esc(x.client_name || "Sem cliente")}<small>${esc(x.contract_name || "")}</small></td><td><div class="operations-progress"><span style="width:${Math.max(0, Math.min(100, Number(x.progress) || 0))}%"></span></div><small>${Number(x.progress) || 0}%</small></td><td>${esc(labels[x.status] || x.status)}</td><td><button class="compact-action" data-progress="${x.id}" type="button">Atualizar progresso</button> <button class="compact-action" data-edit="${x.id}" type="button">Editar</button></td></tr>`).join("") || state("empty", "Nenhum projeto", "Crie um projeto para acompanhar entregas.")}</tbody></table></div></section>`;
    bindCommon("projeto", rows, "/api/projects");
    dashboardGrid.querySelector("[data-project-search]")?.addEventListener("input", (event) => { projectState.query = event.target.value; clearTimeout(projectState.timer); projectState.timer = setTimeout(renderProjectsConnected, 250); });
    dashboardGrid.querySelector("[data-project-status]")?.addEventListener("change", (event) => { projectState.status = event.target.value; renderProjectsConnected(); });
