...
const integrationLabel = { whatsapp: "WhatsApp / Evolution API", email: "E-mail", smtp: "SMTP", google_calendar: "Google Calendar", google_drive: "Google Drive", github: "GitHub", n8n: "n8n", asaas: "Asaas", stripe: "Stripe", firebase: "Firebase", supabase: "Supabase", cnpj: "Consulta CNPJ", esign: "Assinatura eletrônica", webhook: "Webhook", api: "API própria" };
...
function integrationForm(item = null) {
  ...
  const values = { ...item, ...protectedConfig, webhookUrl: protectedConfig.webhookUrl || protectedConfig.url || "" };
...
async function renderIntegrationsScreen() {
  ...
  try {
    ...
    dashboardGrid.innerHTML = `${ui.header({ kicker: "Configuração", title, description, actions: ui.button({ label: "+ Nova conexão", attr: "data-integration-new" }) })}${ui.stats([{ label: "Conexões", value: ui.number(items.length) }, { label: "Ativas", value: ui.number(connected), tone: "green" }, { label: "Pendentes", value: ui.number(items.length - connected - errors) }, { label: "Com erro", value: ui.number(errors), tone: "red" }])}<section class="data-card automation-list"><div class="integration-notice"><span aria-hidden="true">ⓘ</span><p>Cadastre a conexão, salve e depois use “Testar”. O sistema só marca como ativa quando recebe uma resposta confirmada.</p></div>${ui.toolbar({ search: { value: integrationState.search, placeholder: "Buscar serviço ou conexão…" }, filters: [{ key: "status", label: "Filtrar status", value: integrationState.status, options: [["", "Todos"], ["connected", "Ativas"], ["disconnected", "Não testadas"], ["error", "Com erro"]] }] })}<div class="integration-grid">${items.length ? items.map((item) => { const [status, tone] = statusInfo(item.status); return `<article class="data-card integration-card"><div class="section-heading"><div><h3>${integrationEsc(integrationLabel[item.provider] || item.provider || "Provedor")}</h3><small>${integrationEsc(item.account_name || "Conexão sem nome")}${item.environment ? ` · ${integrationEsc(item.environment)}` : ""}</small></div><span class="finance-status ${tone}">${status}</span></div><small class="integration-sync">${item.last_sync_at ? `Verificada ${integrationEsc(ui.relative(item.last_sync_at))}` : "Ainda não verificada"}${item.last_error ? ` · ${integrationEsc(item.last_error)}` : ""}</small><div class="template-actions"><button class="compact-action" data-integration-details="${integrationEsc(item.id)}" type="button">Detalhes</button><button class="compact-action" data-integration-test="${integrationEsc(item.id)}" type="button">Testar</button><button class="compact-action" data-integration-edit="${integrationEsc(item.id)}" type="button">Editar</button><button class="compact-action" data-integration-delete="${integrationEsc(item.id)}" type="button">Excluir</button></div><small data-integration-result="${integrationEsc(item.id)}" class="integration-result" role="status" aria-live="polite"></small></article>`; }).join("") : ui.empty({ title: "Nenhuma conexão cadastrada", text: "Comece pelo botão Nova conexão e escolha o serviço.", cta: "Nova conexão", attr: "data-integration-empty" })}</div><div class="table-pagination"><button type="button" class="compact-action" data-integration-prev ${integrationState.offset === 0 ? "disabled" : ""}>Anterior</button><span>Página ${Math.floor(integrationState.offset / 100) + 1}</span><button type="button" class="compact-action" data-integration-next ${(pagination.returned ?? items.length) < 100 ? "disabled" : ""}>Próxima</button></div></section>`;
