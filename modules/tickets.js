...
async function renderTicketsScreen() {
  ...
  try {
    ...
    const ticketStatusField = createConfig.ticket?.fields?.find((field) => field.name === "status");
    if (!ticketStatusField) throw new Error("O formulário de ticket está incompleto. Atualize a tela e tente novamente.");
    ticketStatusField.options = Object.entries(ticketLabels);
