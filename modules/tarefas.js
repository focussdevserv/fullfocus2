...
const tarefaPriorityField = createConfig.tarefa?.fields?.find((field) => field.name === "priority");
const tarefaStatusField = createConfig.tarefa?.fields?.find((field) => field.name === "status");
if (tarefaPriorityField) tarefaPriorityField.options = [["low", "Baixa"], ["medium", "Média"], ["high", "Alta"], ["urgent", "Urgente"]];
if (tarefaStatusField) tarefaStatusField.options = [["todo", "Pendente"], ["doing", "Em andamento"], ["waiting_client", "Aguardando cliente"], ["review", "Em revisão"], ["blocked", "Bloqueada"], ["done", "Concluída"], ["cancelled", "Cancelada"]];
