...
async function renderEstrutura(key) {
  ...
  try {
    ...
    const statusCount = (status) => rows.filter((row) => String(row.status || "").toLowerCase() === status).length;
    const pendingCount = statusCount("pending") + statusCount("draft");
    const completeCount = statusCount("completed") + statusCount("approved") + statusCount("published");
    const structuredStats = ui.stats([
      { label: "Registros nesta pÃ¡gina", value: ui.number(rows.length) },
      { label: "Pendentes", value: ui.number(pendingCount), tone: pendingCount ? "orange" : "green" },
      { label: "ConcluÃ­dos", value: ui.number(completeCount), tone: "green" },
      { label: "PÃ¡gina", value: ui.number(Math.floor(filterState.offset / 100) + 1) }
    ]);
    ...
    dashboardGrid.querySelector(".page-intro")?.insertAdjacentHTML("afterend", structuredStats);
...
async function renderChangeRequests() {
  ...
  try {
    ...
    dashboardGrid.querySelectorAll("[data-change-edit]").forEach((button) => button.addEventListener("click", async () => { const row = rows.find((item) => String(item.id) === String(button.dataset.changeEdit)); if (!row) return; await prepareStructuredCreate("alteracao"); const form = document.querySelector("#create-dialog-form"); if (!form) return; Object.entries(row).forEach(([field, value]) => { const input = form.elements[field]; if (input && value !== null && value !== undefined) input.value = String(value); }); form.dataset.method = "PATCH"; form.dataset.endpoint = `/api/change_requests/${row.id}`; document.querySelector("#dialog-title").textContent = "Editar alteração"; document.querySelector("#dialog-kicker").textContent = "Editar alteração"; }));
...
async function renderDeliveries() {
  ...
  try {
    ...
    dashboardGrid.querySelectorAll("[data-delivery-edit]").forEach((button) => button.addEventListener("click", async () => { const row = rows.find((item) => String(item.id) === String(button.dataset.deliveryEdit)); if (!row) return; await prepareStructuredCreate("entrega"); const form = document.querySelector("#create-dialog-form"); if (!form) return; Object.entries(row).forEach(([field, value]) => { const input = form.elements[field]; if (input && value !== null && value !== undefined) input.value = String(value); }); form.dataset.method = "PATCH"; form.dataset.endpoint = `/api/deliveries/${row.id}`; document.querySelector("#dialog-title").textContent = "Editar entrega"; document.querySelector("#dialog-kicker").textContent = "Editar entrega"; }));
...
async function renderVault() {
  ...
  try {
    ...
    dashboardGrid.querySelectorAll("[data-vault-edit]").forEach((button) => button.addEventListener("click", async () => { const row = rows.find((item) => String(item.id) === String(button.dataset.vaultEdit)); if (!row) return; await prepareStructuredCreate("cofre"); const form = document.querySelector("#create-dialog-form"); if (!form) return; Object.entries(row).forEach(([field, value]) => { const input = form.elements[field]; if (input && value !== null && value !== undefined && field !== "secret") input.value = String(value); }); form.dataset.method = "PATCH"; form.dataset.endpoint = `/api/vault/${row.id}`; document.querySelector("#dialog-title").textContent = "Editar acesso"; document.querySelector("#dialog-kicker").textContent = "Editar acesso"; }));
