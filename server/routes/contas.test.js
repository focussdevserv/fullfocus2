...
function harness() {
  ...
  const pool = { query: async (sql, params) => {
    ...
    if (sql.startsWith("select coalesce((select sum(p.amount)")) return { rows: [{ received_total: 75, balance_open: 125, confirmed_revenue: 90, open_tickets: 37, pending_changes: 42 }] };
...
test("overview usa agregados completos além dos limites da listagem", async () => { const h = harness(); const response = await request(h, "/api/clients/7/overview"); assert.equal(response.status, 200); const body = await response.json(); assert.equal(body.client.name, "Cliente teste"); assert.deepEqual(Object.keys(body), ["client", "contacts", "conversations", "contracts", "proposals", "receivables", "payments", "revenues", "projects", "tasks", "tickets", "files", "briefings", "change_requests", "infrastructure", "activities", "summary"]); assert.equal(body.proposals[0].title, "Proposta direta"); assert.equal(body.payments.length, 30); assert.equal(body.tickets.length, 30); assert.equal(body.change_requests.length, 30); assert.equal(body.summary.received_total, 75); assert.equal(body.summary.balance_open, 125); assert.equal(body.summary.confirmed_revenue, 90); assert.equal(body.summary.open_tickets, 37); assert.equal(body.summary.pending_changes, 42); const summaryCall = h.calls.find((x) => x.sql.startsWith("select coalesce((select sum(p.amount)")); assert.ok(summaryCall); assert.doesNotMatch(summaryCall.sql, /\blimit\b/i); assert.match(summaryCall.sql, /from tickets t where t\.organization_id=\$1 and t\.client_id=\$2/); assert.match(summaryCall.sql, /from change_requests cr where cr\.organization_id=\$1 and cr\.client_id=\$2/); assert.deepEqual(summaryCall.params, [org, 7]); const proposalCall = h.calls.find((x) => x.sql.startsWith("select p.id,p.title")); assert.match(proposalCall.sql, /p\.client_id=\$4/); assert.deepEqual(proposalCall.params, [org, null, null, 7]); assert.ok(h.calls.every((x) => x.sql.includes("organization_id"))); });
