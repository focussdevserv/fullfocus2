...
function harness() {
  ...
  const pool = { query: async (sql, params) => { calls.push({ sql, params }); if (sql.startsWith("select id from clients")) return { rowCount: 1, rows: [{ id: params[0] }] }; if (sql.startsWith("insert into client_portal_links")) return { rowCount: 1, rows: [] }; if (sql.startsWith("select client_id, organization_id")) return { rowCount: params[0] === invalidHash ? 0 : 1, rows: [{ client_id: 7, organization_id: org }] }; if (sql.startsWith("select name from clients")) return { rowCount: 1, rows: [{ name: "Cliente teste" }] }; if (sql.startsWith("select id,name,status")) return { rows: [{ id: 1, name: "Contrato", status: "active", value: "100" }] }; if (sql.startsWith("select id,description")) return { rows: [] }; return { rows: [] }; } };
