...
function harness() {
  ...
  const pool = { query: async (sql, params) => {
    ...
    if (sql.startsWith("select c.*,co.name company_name")) return { rowCount: 1, rows: [{ id: 7, name: "Cliente teste", company_id: null, contact_id: null }] };
    if (sql.startsWith("select p.id,p.title")) return { rows: [{ id: 91, title: "Proposta direta", status: "sent", amount: "500" }] };
    ...
    if (sql.startsWith("select client_id, organization_id")) return { rowCount: params[0] === invalidHash ? 0 : 1, rows: [{ client_id: 7, organization_id: org }] };
    if (sql.startsWith("select name from clients")) return { rowCount: 1, rows: [{ name: "Cliente teste" }] };
    if (sql.startsWith("select id,name,status")) return { rows: [{ id: 1, name: "Contrato", status: "active", value: "100" }] };
...
test("overview agrega operações vinculadas aos projetos sem duplicar vínculo direto", async () => {
  ...
  for (const prefix of ["select id,name,url,kind", "select id,name,status,project_id", "select id,title,status,project_id,client_id", "select id,kind,name,provider"]) {
    assert.equal(h.calls.some((item) => item.sql.startsWith(prefix)), false, `consulta inicial indevida ${prefix}`);
  }
});
...
test("overview limits and payment aggregate", async () => {
  const h = harness();
  const response = await request(h, "/api/clients/7/overview");
  assert.equal(response.status, 200);
  for (const prefix of ["select id,name,email,phone,role from contacts", "select id,name,status,value,total_value", "select id,name,stage,amount,probability", "select id,name,status,progress"]) {
    const call = h.calls.find((item) => item.sql.startsWith(prefix));
    assert.ok(call, `consulta ${prefix}`);
    assert.match(call.sql, /limit 20$/i);
  }
  const summaryCall = h.calls.find((item) => item.sql.startsWith("select coalesce((select sum(p.amount)"));
  assert.ok(summaryCall);
  assert.match(summaryCall.sql, /join receivables pr on pr\.id=p\.receivable_id/);
  assert.match(summaryCall.sql, /pr\.client_id=\$2/);
});
