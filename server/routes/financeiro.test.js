...
function setup() {
  const calls = [];
  const pool = { query: async (sql, params) => { calls.push({ sql, params }); if (sql.includes("generate_series")) return { rows: [{ month: "2026-08", revenue: "10", expense: "3", result: "7" }] }; if (sql.startsWith("select s.*")) return { rows: [{ id: 1, organization_id: ORG, plan: "Pro", amount: "20", status: "active" }] }; if (sql.startsWith("insert into subscriptions")) return { rows: [{ id: 2, organization_id: ORG, plan: "Pro", amount: "20" }] }; return { rows: [] }; }, connect: async () => ({ query: async (sql, params) => { calls.push({ sql, params }); if (sql.startsWith("select * from contracts where")) return { rowCount: 1, rows: [{ id: 9, organization_id: ORG, client_id: 7, status: "active", name: "Site", value: "2700", down_payment: "300", installments: 2, installment_value: "1200", payment_method: "pix" }] }; if (sql.startsWith("select * from payables where")) return { rowCount: 1, rows: [{ id: 11, organization_id: ORG, description: "Servidor", amount: "120", supplier: "Cloud", status: "pending", due_at: "2026-09-20" }] }; if (sql.startsWith("insert into receivables")) return { rowCount: 1, rows: [{ id: calls.filter((call) => call.sql.startsWith("insert into receivables")).length }] }; if (sql.startsWith("insert into expenses")) return { rowCount: 1, rows: [{ id: 12, amount: "120", status: "paid" }] }; if (sql.startsWith("update payables")) return { rowCount: 1, rows: [{ id: 11, status: "paid" }] }; return { rowCount: 0, rows: [] }; }, release() {} }) };
