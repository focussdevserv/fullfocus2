  ...
  app.get("/api/clients/:id/overview", async (req, res) => {
    ...
    try {
      ...
      const [contacts, conversations, contracts, proposals, opportunities, receivables, payments, revenues, projects, tasks, tickets, files, briefings, changeRequests, infrastructure, activities, financialSummary] = await Promise.all([
        ...
        pool.query("select id,name,url,kind,size_bytes,project_id,client_id,created_at from files where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by created_at desc limit 40", [org, row.id]),
        pool.query("select id,name,status,project_id,client_id,created_at,updated_at from briefings where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by updated_at desc limit 20", [org, row.id]),
        pool.query("select id,title,status,project_id,client_id,description,impact_days,additional_cost,created_at from change_requests where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by created_at desc limit 30", [org, row.id]),
        pool.query("select id,kind,name,provider,project_id,client_id,expires_on,cost,client_price,responsible,status from infrastructure_assets where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by expires_on nulls last,name limit 40", [org, row.id]),
      ...
      const overviewRows = { contacts: uniqueRows(contacts.rows), conversations: uniqueRows(conversations.rows), contracts: uniqueRows(contracts.rows), proposals: uniqueRows(proposals.rows), opportunities: uniqueRows(opportunities.rows), receivables: uniqueRows(receivables.rows), payments: uniqueRows(payments.rows), revenues: uniqueRows(revenues.rows), projects: uniqueRows(projects.rows), tasks: uniqueRows(tasks.rows), tickets: uniqueRows(tickets.rows), files: uniqueRows(files.rows), briefings: uniqueRows(briefings.rows), change_requests: uniqueRows(changeRequests.rows), infrastructure: uniqueRows(infrastructure.rows), activities: uniqueRows(activities.rows) };
