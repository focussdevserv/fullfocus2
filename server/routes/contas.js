...
export function register(app, ctx) {
  ...
  app.get("/api/clients/:id/overview", async (req, res) => {
    ...
    try {
      ...
      const [contacts, conversations, contracts, proposals, receivables, projects, tasks, tickets, files, briefings, changeRequests, infrastructure, activities] = await Promise.all([
      ...
      res.json({ client: row, contacts: contacts.rows, conversations: conversations.rows, contracts: contracts.rows, proposals: proposals.rows, receivables: receivables.rows, projects: projects.rows, tasks: tasks.rows, tickets: tickets.rows, files: files.rows, briefings: briefings.rows, change_requests: changeRequests.rows, infrastructure: infrastructure.rows, activities: activities.rows, summary: { active_projects: projects.rows.filter((item) => !["done", "published", "cancelled"].includes(item.status)).length, completed_projects: projects.rows.filter((item) => ["done", "published"].includes(item.status)).length, balance_open: unpaid, recurring_monthly: Number(row.monthly_fee || 0), open_tickets: tickets.rows.filter((item) => !["done", "closed", "cancelled"].includes(item.status)).length, pending_changes: changeRequests.rows.filter((item) => ["pending", "awaiting_approval"].includes(item.status)).length } });
