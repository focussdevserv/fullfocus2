...
export function register(app, ctx) {
  ...
  app.get("/api/clients/:id/overview", async (req, res) => {
    ...
    try {
      ...
      const [contacts, conversations, contracts, proposals, opportunities, receivables, payments, revenues, projects, tasks, tickets, activities, financialSummary] = await Promise.all([
        pool.query("select id,name,email,phone,role from contacts where organization_id=$1 and (id=$2 or company_id=$3) order by name limit 20", [org, row.contact_id || 0, row.company_id || 0]),
        ...
        pool.query("select id,name,status,value,total_value,proposal_id,project_id,starts_on,ends_on from contracts where organization_id=$1 and client_id=$2 order by created_at desc limit 20", [org, row.id]),
        ...
        pool.query("select id,name,stage,amount,probability,expected_close,client_id,created_at from opportunities where organization_id=$1 and client_id=$2 order by created_at desc limit 20", [org, row.id]),
        ...
        pool.query("select id,name,status,progress,client_id,contract_id,due_on,production_url,repository_url from projects where organization_id=$1 and client_id=$2 order by created_at desc limit 20", [org, row.id]),
        ...
        pool.query("select coalesce((select sum(p.amount) from payments p join receivables r on r.id=p.receivable_id and r.organization_id=p.organization_id where p.organization_id=$1 and r.client_id=$2),0)::float8 received_total,coalesce((select sum(greatest(r.amount-coalesce(paid.amount,0),0)) from receivables r left join (select p.receivable_id,sum(p.amount) amount from payments p join receivables pr on pr.id=p.receivable_id and pr.organization_id=p.organization_id where p.organization_id=$1 and pr.client_id=$2 group by p.receivable_id) paid on paid.receivable_id=r.id where r.organization_id=$1 and r.client_id=$2),0)::float8 balance_open,coalesce((select sum(rv.amount) from revenues rv where rv.organization_id=$1 and rv.client_id=$2 and rv.status='confirmed'),0)::float8 confirmed_revenue,(select count(*)::int from tickets t where t.organization_id=$1 and t.client_id=$2 and (t.status is null or t.status not in ('done','closed','cancelled'))) open_tickets,(select count(*)::int from change_requests cr where cr.organization_id=$1 and cr.client_id=$2 and cr.status in ('pending','awaiting_approval')) pending_changes,(select count(*)::int from opportunities o where o.organization_id=$1 and o.client_id=$2) opportunities_total,(select count(*)::int from opportunities o where o.organization_id=$1 and o.client_id=$2 and o.stage not in ('won','lost')) open_opportunities", [org, row.id]),
      ]);
      const totals = financialSummary.rows[0] || {};
      const uniqueRows = (items) => { const seen = new Set(); return items.filter((item) => { if (item?.id == null) return true; const key = String(item.id); if (seen.has(key)) return false; seen.add(key); return true; }); };
      const overviewRows = { contacts: uniqueRows(contacts.rows), conversations: uniqueRows(conversations.rows), contracts: uniqueRows(contracts.rows), proposals: uniqueRows(proposals.rows), opportunities: uniqueRows(opportunities.rows), receivables: uniqueRows(receivables.rows), payments: uniqueRows(payments.rows), revenues: uniqueRows(revenues.rows), projects: uniqueRows(projects.rows), tasks: uniqueRows(tasks.rows), tickets: uniqueRows(tickets.rows), files: [], briefings: [], change_requests: [], infrastructure: [], activities: uniqueRows(activities.rows) };
      res.json({ client: row, ...overviewRows, summary: { active_projects: overviewRows.projects.filter((item) => !["done", "published", "cancelled"].includes(item.status)).length, completed_projects: overviewRows.projects.filter((item) => ["done", "published"].includes(item.status)).length, balance_open: Number(totals.balance_open || 0), received_total: Number(totals.received_total || 0), confirmed_revenue: Number(totals.confirmed_revenue || 0), recurring_monthly: Number(row.monthly_fee || 0), open_tickets: Number(totals.open_tickets || 0), pending_changes: Number(totals.pending_changes || 0), opportunities_total: Number(totals.opportunities_total || 0), open_opportunities: Number(totals.open_opportunities || 0) } });
    } catch (e) { const { status, error } = classifyDbError(e, "Não foi possível carregar o resumo do cliente."); res.status(status).json({ error }); }
  });
  app.get("/api/clients/:id/overview/section/:section", async (req, res) => {
    const org = tenant(req, res); if (!org) return;
    const queries = {
      tasks: "select id,title,status,priority,due_at,project_id,client_id from tasks where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by due_at nulls last,created_at desc limit 30",
      files: "select id,name,url,kind,size_bytes,project_id,client_id,created_at from files where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by created_at desc limit 40",
      briefings: "select id,name,status,project_id,client_id,created_at,updated_at from briefings where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by updated_at desc limit 20",
      change_requests: "select id,title,status,project_id,client_id,description,impact_days,additional_cost,created_at from change_requests where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by created_at desc limit 30",
      infrastructure: "select id,kind,name,provider,project_id,client_id,expires_on,cost,client_price,responsible,status from infrastructure_assets where organization_id=$1 and (client_id=$2 or project_id in (select id from projects where organization_id=$1 and client_id=$2)) order by expires_on nulls last,name limit 40",
    };
