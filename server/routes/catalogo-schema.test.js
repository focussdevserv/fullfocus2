...
test("catalog route uses only columns represented by the catalog migrations", () => {
  ...
  for (const obsolete of ["archived", "long_description", "delivery_time", "pages_or_modules", "revisions_included", "entry_value"]) assert.equal(FIELDS.includes(obsolete), false, `obsolete ${obsolete}`);
});
