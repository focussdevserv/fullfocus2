...
const publicProposalQuery = "select p.id,p.organization_id,p.title,p.amount,p.final_amount,p.discount,p.additional_fees,p.down_payment,p.balance_remaining,p.installments,p.installment_amount,p.valid_until,p.notes,p.presentation,p.scope_included,p.modules,p.integrations,p.technologies,p.status,c.name client_name from proposals p left join clients c on c.id=p.client_id and c.organization_id=p.organization_id where p.public_token_hash=$1 and (p.public_token_created_at is null or p.public_token_created_at > now() - ($2 * interval '1 second')) and p.status in ('sent','viewed','negotiation','accepted')";
...
export function registerProposalPublicRoutes(app, { pool, tenant, requireAuth, classifyDbError }) {
  ...
  app.post("/api/proposals/public/:token/decision", async (req, res) => {
    ...
    try {
      ...
      const q = await db.query("update proposals set status=$1,decided_at=now(),approver_name=$2,approver_email=$3,acceptance_comment=$4,accepted_terms=$5,acceptance_ip=$6,updated_at=now() where public_token_hash=$7 and (public_token_created_at is null or public_token_created_at > now() - ($8 * interval '1 second')) and status in ('sent','viewed','negotiation') returning id,title,status,organization_id,opportunity_id,lead_id", [decision, name, email, comment || null, acceptedTerms, req.ip || null, hashToken(req.params.token), publicLinkTtlSeconds()]);
      if (!q.rowCount) { await db.query("rollback"); return res.status(404).json({ error: "Proposta não encontrada, já decidida ou indisponível." }); }
      const proposal = q.rows[0];
      ...
      return res.json({ proposal: { id: proposal.id, title: proposal.title, status: proposal.status }, decided: true });
