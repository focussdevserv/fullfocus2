...
class CommercialFixture {
  ...
  query = async (sql, params = []) => {
    ...
    if (sql.startsWith("insert into contracts")) { const row = this.fromInsert(sql, params, "contracts"); row.id = this.next.contracts++; row.status ||= "draft"; this.state.contracts.push(row); return result([clone(row)]); }
