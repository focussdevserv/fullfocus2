...
test("projeto rejeita cliente diferente do contrato", async (t) => withServer(t, async (calls) => { const response = await request(t, "POST", "/api/projects", { name: "Projeto", client_id: 5, contract_id: 9 }); assert.equal(response.status, 400); assert.match((await response.json()).error, /mesmo do contrato/i); assert.ok(calls.some((call) => call.sql.includes("from contracts where id=$1"))); }));
test("edição de projeto valida cliente e contrato em conjunto", async (t) => withServer(t, async (calls) => { const response = await request(t, "PATCH", "/api/projects/2", { client_id: 5, contract_id: 9 }); assert.equal(response.status, 400); assert.match((await response.json()).error, /mesmo do contrato/i); assert.equal(calls.filter((call) => call.sql.startsWith("update projects")).length, 0); }));
test("numeração automática de contrato usa lock transacional e não count plus one", async (t) => withServer(t, async (calls) => { const response = await request(t, "POST", "/api/contracts", { name: "Contrato", client_id: 4, value: 2700 }); assert.equal(response.status, 201); const insert = calls.find((call) => call.sql.includes("insert into contracts")); assert.ok(insert); assert.match(insert.sql, /pg_advisory_xact_lock/); assert.doesNotMatch(insert.sql, /count\(\*\)/i); assert.match(insert.sql, /regexp_match\(contract_number/); }));
