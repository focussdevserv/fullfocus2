...
test("contrato rejeita proposta que nÃ£o estÃ¡ aprovada", async (t) => withServer(t, async () => { const response = await request(t, "POST", "/api/contracts", { name: "Contrato", client_id: 4, value: 2700, proposal_id: 7 }); assert.equal(response.status, 400); assert.match((await response.json()).error, /proposta precisa estar aprovada/i); }));
