...
export function register(app, ctx) {
  ...
  app.get("/api/leads", async (req, res) => {
    ...
    if (req.query.status === "open") where.push("l.status not in ('won','lost')");
  ...
  app.get("/api/opportunities", async (req, res) => {
    const org = tenant(req, res); if (!org) return;
    const values = [org], where = ["o.organization_id=$1"];
    const add = (sql, value) => { values.push(value); where.push(sql.replace("$VALUE", `$${values.length}`)); };
    const search = asText(req.query.search || req.query.q);
  ...
  app.get("/api/proposals", async (req, res) => {
    ...
    const limit = Math.min(Math.max(Number(req.query.limit) || 100, 1), 250), offset = Math.max(Number(req.query.offset) || 0, 0);
    values.push(limit, offset);
    try { const q = await pool.query(`select p.*, o.name opportunity_name, l.name lead_name, c.name client_name, pr.name project_name, (select coalesce(sum(quantity*unit_price),0)::float8 from proposal_items i where i.proposal_id=p.id and i.organization_id=p.organization_id) items_total, (select count(*)::int from proposal_items i where i.proposal_id=p.id and i.organization_id=p.organization_id) items_count from proposals p left join opportunities o on o.id=p.opportunity_id and o.organization_id=p.organization_id left join leads l on l.id=p.lead_id and l.organization_id=p.organization_id left join clients c on c.id=p.client_id and c.organization_id=p.organization_id left join projects pr on pr.id=p.project_id and pr.organization_id=p.organization_id where ${where.join(" and ")} order by p.created_at desc limit $${values.length - 1} offset $${values.length}`, values); res.json({ proposals: q.rows, pagination: { limit, offset, returned: q.rows.length } }); } catch (e) { fail(res, e, "Não foi possível carregar as propostas."); }
  });
  ...
  app.post("/api/proposals/:id/recalculate", async (req, res) => {
    const org = tenant(req, res); if (!org) return;
    const discount = Number(req.body?.discount || 0), additionalFees = Number(req.body?.additional_fees || 0), downPayment = Number(req.body?.down_payment || 0), installments = Number(req.body?.installments || 1);
    if (![discount, additionalFees, downPayment, installments].every(Number.isFinite) || discount < 0 || additionalFees < 0 || downPayment < 0 || !Number.isInteger(installments) || installments < 1) return bad(res, "Informe desconto, taxas, entrada e parcelas válidos.");
    try { const proposal = await pool.query("select id from proposals where id=$1 and organization_id=$2", [req.params.id, org]); if (!proposal.rowCount) return res.status(404).json({ error: "Proposta não encontrada." }); const subtotal = await proposalTotal(req.params.id, org), finalAmount = subtotal - discount + additionalFees; if (finalAmount < 0 || downPayment > finalAmount) return bad(res, "A entrada não pode exceder o valor final."); const balance = finalAmount - downPayment, installmentAmount = balance / installments; const q = await pool.query("update proposals set amount=$1,discount=$2,additional_fees=$3,final_amount=$1,down_payment=$4,balance_remaining=$5,installments=$6,installment_amount=$7,updated_at=now() where id=$8 and organization_id=$9 returning *", [finalAmount, discount, additionalFees, downPayment, balance, installments, installmentAmount, req.params.id, org]); res.json({ proposal: { ...q.rows[0], subtotal, final_amount: finalAmount, balance_remaining: balance, installment_amount: installmentAmount } }); } catch (e) { fail(res, e, "Não foi possível recalcular a proposta."); }
  });
  app.get("/api/proposals/:id", async (req, res) => { const org = tenant(req, res); if (!org) return; try { const p = await pool.query("select p.*, o.name opportunity_name, l.name lead_name, l.email lead_email, l.phone lead_phone, c.name client_name, pr.name project_name from proposals p left join opportunities o on o.id=p.opportunity_id and o.organization_id=p.organization_id left join leads l on l.id=p.lead_id and l.organization_id=p.organization_id left join clients c on c.id=p.client_id and c.organization_id=p.organization_id left join projects pr on pr.id=p.project_id and pr.organization_id=p.organization_id where p.id=$1 and p.organization_id=$2", [req.params.id, org]); if (!p.rowCount) return res.status(404).json({ error: "Proposta não encontrada." }); const items = await pool.query("select * from proposal_items where proposal_id=$1 and organization_id=$2 order by position, id", [req.params.id, org]); res.json({ proposal: { ...p.rows[0], items: items.rows, items_total: items.rows.reduce((a, i) => a + Number(i.quantity) * Number(i.unit_price), 0) } }); } catch (e) { fail(res, e, "Não foi possível carregar a proposta."); } });
