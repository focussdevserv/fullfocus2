...
export function createPortalAccessGuard({ pool }) {
  return async (req, res, next) => {
    ...
    try {
      const q = await pool.query("select id,client_id,organization_id,portal_auth_enabled from client_portal_links where token_hash=$1 and (expires_at is null or expires_at > now())", [tokenHash(req.params.token)]);
      const link = q.rows[0];
