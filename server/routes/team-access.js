export function register(app, ctx) {
  ...
  app.get("/api/team/access-status", async (req, res) => { const org = tenant(req, res); if (!org) return; const search = text(req.query?.search || req.query?.q), status = text(req.query?.status), department = text(req.query?.department), availability = text(req.query?.availability); const params = [org], where = ["u.organization_id=$1"]; const add = (sql, value) => { params.push(value); where.push(sql.replace("$VALUE", `$${params.length}`)); }; if (search) add("(u.name ilike '%' || $VALUE || '%' or u.email ilike '%' || $VALUE || '%' or coalesce(u.job_title,'') ilike '%' || $VALUE || '%')", search); if (["active", "inactive", "blocked"].includes(status)) add("coalesce(u.access_status,'active')=$VALUE", status); if (department) add("coalesce(u.department,'')=$VALUE", department); if (["online", "available", "busy", "offline"].includes(availability)) add("coalesce(u.availability,'offline')=$VALUE", availability); const limit = Math.min(Math.max(Number(req.query?.limit) || 250, 1), 250), offset = Math.max(Number(req.query?.offset) || 0, 0); params.push(limit, offset); try { const q = await pool.query(`select u.id,u.name,u.email,u.role,u.team_role_id,u.department,u.job_title,u.availability,u.access_status,u.access_expires_on,u.created_at,tr.name as team_role_name from users u left join team_roles tr on tr.id=u.team_role_id and tr.organization_id=u.organization_id where ${where.join(" and ")} order by u.created_at limit $${params.length - 1} offset $${params.length}`, params); res.json({ users: q.rows, pagination: { limit, offset, returned: q.rows.length } }); } catch { res.status(503).json({ error: "Unable to load team members." }); } });
