...
// Links de briefing só ficam públicos depois da publicação. Esta guarda também
// protege as rotas legadas abaixo, que ainda consultam o status no SQL.
app.use(["/api/briefings/public/:token", "/briefing/:token"], async (req, res, next) => {
  try {
    const q = await pool.query("select status from briefings where public_token=$1", [req.params.token]);
    if (!q.rowCount || !["published", "sent"].includes(q.rows[0].status)) return res.status(404).json({ error: "Briefing indisponível." });
    next();
  } catch {
    res.status(503).json({ error: "Não foi possível validar o briefing." });
  }
});
