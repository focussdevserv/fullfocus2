...
app.use("/api/events", async (req, res, next) => { if (!['POST', 'PATCH'].includes(req.method)) return next(); const org = tenant(req, res); if (!org) return; const body = req.body || {}, values = { client_id: body.client_id ?? body.clientId, project_id: body.project_id ?? body.projectId, contract_id: body.contract_id ?? body.contractId }; try { await validateRelations(values, org); return next(); } catch (e) { if (e.code === "invalid_relation") return res.status(400).json({ error: e.message }); return res.status(503).json({ error: "NÃ£o foi possÃ­vel validar os vÃ­nculos do evento." }); } });
