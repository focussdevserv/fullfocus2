...
app.post("/api/satisfaction/:token", async (req, res) => { const rating = Number(req.body?.rating), comment = asText(req.body?.comment).slice(0, 4000); if (!Number.isInteger(rating) || rating < 1 || rating > 5) return res.status(400).json({ error: "Informe uma nota de 1 a 5." }); try { const q = await pool.query("update satisfaction_requests set status='completed',rating=$1,comment=$2,responded_at=now() where token=$3 and status='pending' returning id,status,rating,comment,responded_at", [rating, comment || null, req.params.token]); if (!q.rowCount) return res.status(404).json({ error: "Pesquisa inexistente ou já respondida." }); res.json({ satisfaction: q.rows[0] }); } catch { res.status(503).json({ error: "Não foi possível registrar a avaliação." }); } });
