...
const entities = {
  ...
  leads: { fields: ["name", "company", "company_id", "contact_id", "email", "phone", "source", "status", "notes", "value", "campaign_id", "owner_id", "tags"], required: ["name"] }, opportunities: { fields: ["name", "lead_id", "company_id", "contact_id", "client_id", "stage", "amount", "expected_close", "notes", "probability", "owner_id", "tags"], required: ["name"] },
...
// Página individual compartilhável do catálogo: funciona mesmo quando o catálogo geral é aberto em outro canal.
app.get("/catalog-item/:organizationId/:id", async (req, res) => {
  if (!/^[0-9a-f-]{36}$/i.test(req.params.organizationId) || !/^\d+$/.test(req.params.id)) return res.status(400).send("Item inválido.");
  const safe = (value) => String(value ?? "").replace(/[&<>\"]/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;" }[character]));
  try {
    const q = await pool.query("select name,kind,price,unit,description,short_description,category,image_url,delivery_days,features,benefits from catalog_items where id=$1 and organization_id=$2 and active=true and public_visible=true and archived_at is null", [req.params.id, req.params.organizationId]);
    if (!q.rowCount) return res.status(404).send("Produto não encontrado.");
    const item = q.rows[0], url = `${PUBLIC_APP_URL}/catalog-item/${req.params.organizationId}/${req.params.id}`, image = item.image_url || `${PUBLIC_APP_URL}/assets/catalog/catalog-digital.png`, price = Number(item.price || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 });
    const text = `Confira ${item.name} da Focussdev: ${url}`;
    const list = Array.isArray(item.features) ? item.features : Array.isArray(item.benefits) ? item.benefits : [];
    res.type("html").send(`<!doctype html><html lang="pt-BR"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta property="og:title" content="${safe(item.name)} · Focussdev"><meta property="og:description" content="${safe(item.short_description || item.description || "Solução digital Focussdev")}"><meta property="og:image" content="${safe(image)}"><title>${safe(item.name)} · Focussdev</title><style>:root{color-scheme:dark;font-family:system-ui,sans-serif;background:#070b14;color:#f8fafc}body{margin:0;padding:24px 16px;background:radial-gradient(circle at 80% 0,#153b77,#070b14 55%)}main{max-width:900px;margin:auto}.brand{color:#38a8ff;font-size:24px;font-weight:800}.brand span{color:#fff}.card{margin-top:24px;overflow:hidden;border:1px solid #334766;border-radius:20px;background:#111b2e;box-shadow:0 16px 45px #0005}.hero{width:100%;height:320px;object-fit:cover}.content{padding:26px}.kind,.muted{color:#94a3b8}.price{font-size:32px;font-weight:800;margin:18px 0}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:24px}a,button{border:0;border-radius:10px;padding:12px 16px;background:#1688ff;color:white;font:inherit;font-weight:700;text-decoration:none;cursor:pointer}a.secondary,button.secondary{background:#263b5d}li{margin:8px 0;color:#dbeafe}</style><main><div class="brand">Focuss<span>dev</span></div><article class="card"><img class="hero" src="${safe(image)}" alt="${safe(item.name)}" loading="eager"><div class="content"><p class="kind">${safe(item.category || item.kind || "Solução digital")}</p><h1>${safe(item.name)}</h1><p class="muted">${safe(item.description || item.short_description || "")}</p><div class="price">R$ ${price}<small> ${safe(item.unit || "por projeto")}</small></div>${item.delivery_days ? `<p class="muted">Prazo estimado: ${safe(item.delivery_days)} dias</p>` : ""}${list.length ? `<ul>${list.slice(0,12).map((entry) => `<li>${safe(typeof entry === "string" ? entry : entry?.label || entry?.name || "")}</li>`).join("")}</ul>` : ""}<div class="actions"><a href="https://wa.me/5511969153824?text=${encodeURIComponent(`Olá! Tenho interesse em ${item.name}.`)}">Falar no WhatsApp</a><a class="secondary" href="mailto:contato@focussdev.art?subject=${encodeURIComponent(`Interesse em ${item.name}`)}">Enviar e-mail</a><button class="secondary" type="button" onclick="navigator.clipboard?.writeText(${JSON.stringify(text)}).then(()=>this.textContent='Link copiado')">Compartilhar</button></div></div></article></main></html>`);
  } catch { res.status(503).send("Não foi possível carregar o item."); }
});
